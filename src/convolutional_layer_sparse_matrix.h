...
*/
void conv_forward_sparse_matrix(const conv_layer_t* l, const vol_t** in, vol_t** out, const int start, const int end) {
  for (int i = start; i <= end; i++) {
    ...
    // fprintf(stderr, "Input size: %d %d %d \n", l->in_sx, l->in_sx, l->in_depth);
    // fprintf(stderr, "Kernel size: %d %d \n", l->sx, l->sx);
    // fprintf(stderr, "Output size: %d %d %d \n\n", l->out_sx, l->out_sy, l->out_depth);
    ...
    function_summary(SparseMatrixMultiplication,
                     M, N, K, 
                     l->sparse_matrix,
                     col_inputs, N,
                     out[i]->w, N);
...
conv_layer_t* make_conv_layer(int in_sx, int in_sy, int in_depth,
  ...
  l->col_inputs = (storage_t*)_mm_malloc(sizeof(storage_t)*sx*sx*l->out_sx*l->out_sy*in_depth,32);
  ...
  l->forward = &conv_forward_sparse_matrix;
...
*/
void conv_load(conv_layer_t* l, const int* params, const weight_t* weights, char* sparse_weights_file_name) {  
  ...
  SparseMatrix* matrix = init_SparseMatrix(sparse_weights_file_name);
  l->sparse_matrix = matrix;
}
