...
void save_matrix_to_file(float* weights, const int M, const int N){
  FILE *f = fopen("cifar_conv1_input.txt", "w");
  if (f == NULL)
  {
      printf("Error opening file!\n");
      exit(1);
  }
  fprintf(stderr, "print the weights of a %d x %d matrix to a file \n", M, N);
